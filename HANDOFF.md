...
> 最終更新: 2026-07-13（選曲品質改修: tasks/triwx-revision-spec.md 変更1〜9を実装）
...
**MVP完成・本番稼働中。** https://triwx.vercel.app で実再生・AI選曲・ナレーション・時報/ジングル・評価・ログ保存・散らし具合レポート（tasks/T4）まで一通り動いている。
...
│  │                            #   （年代レンジ・選択上限/排他・comboHintsもここから導出）
│  ├─ core.ts                   # 方針/候補生成(自己判定つき)、Spotify解決(原曲優先)、ナレーション
...
└─ logs/
   ...
   └─ stats.ts                  # 散らし具合レポート（純関数。/api/log?stats=1 から利用）
```
...
- **fill API（1回=1曲）**: 方針(キャッシュ)→候補生成(LLM、頭出し数曲→各カードに照らして自己判定→合格を確信度順に最大3件。知らない曲は「推測」明記で通す)→解決(アーティスト不一致棄却・バージョン文字列ハード除外・年ズレ減点・原曲優先スコア)→重複/クールダウン/年代/言語ゲート→ナレーション→ログ下書き。失敗時はL2拡大→L3年代緩和→L4リプレイ(地域カード不一致は注入しない)→L5種曲。**503はSpotify検索自体が死んでいるときだけ**。別役の審査(judge)LLMパスは廃止済み（却下回数と人間評価が無相関だったため。tasks/triwx-revision-spec.md 変更6/9-8、PROJECT.md §6）
...
- **散らし具合レポート**（T4・実装済み）: `GET /api/log?stats=1`（`limit`/`sessionId`は既存パラメータ流用可、例: `?stats=1&limit=1000`）。`src/logs/stats.ts`の`computeDiversityStats`が純関数で集計。返る指標: アーティスト重複率+上位重複アーティスト、同一曲7日内重複（件数0が理想。ただしreplay層は意図的にこの窓を跨いで再生するため非0が即異常とは限らない。source内訳と突き合わせて判断）、年代分布（min/max/中央値/十年紀ヒストグラム）、言語分布（JPスクリプト比）、source率、fallbackReason率、judgeRejected平均値、resolveMetaのtitle/artist不一致率、評価（◎○✕）集計、conditionSnapshot（カード構成）単位の内訳。詳細はtasks/T4-add-diversity-report.md参照
...
散らし具合レポート（T4）は実装済み。直近では tasks/triwx-revision-spec.md に沿って選曲品質の改修（時間カードの情景ベース化、性格カード選択上限2枚、地域カード拡充、生成コール内自己判定への統合、バージョン文字列/年ズレのコード側フィルタ等）を実施した（2026-07-13）。**まだ本番の実再生では検証していない**。次の一手は tasks/triwx-revision-spec.md §再テスト計画(9-7)に沿って `midnight + lang_ja`（era=0, popularity=100）等の条件で再テストし、bad率の変化とログ（source/judgeRejected/fallbackReason）を確認すること。あわせて **T1（本番ログでの安定化観測）** も引き続き優先度が高い。
...
- **審査を厳しくする=品質向上ではない**。過去にデッドロック→条件無視曲の混入を起こした（PROJECT.md §6）。**別役の審査(judge)LLMパスは2026-07-13に廃止済み**。`judgeCandidates`のような別関数を復活させず、自己判定は`generateCandidates`内で完結させること
- **性格カードは選択上限2枚、地域・言語カードは1枚**（`MAX_PERSONALITY_CARDS`/`MAX_REGION_CARDS`、`src/pipeline/definitions.ts`が単一ソース）。UIの選択制御を変えるときは必ずここを経由し、page.tsx側にロジックを複製しないこと
